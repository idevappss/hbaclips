...
function renderClips() {
  ...
  for (const p of state.projects || []) if (state.projectIds.has(p.id)) rows.push(`<li><span class="tag">${letter()}</span><span class="name">${esc(p.name)}</span><span class="size">${fmtDur(p.duration)} · HBA Content Backend</span><button class="x" type="button" data-project="${p.id}" aria-label="Remove">×</button></li>`);
...
function renderLibrary() {
  ...
  lib.innerHTML = state.projects.length
    ...
    : `<small>No HBA Content Backend videos yet.</small>`;
}
