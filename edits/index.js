...
// caches its database per process, so writing through it from a second server would clobber Clip Studio's copy.
...
let pumping = false;
// false when a host (Clip Studio) renders plans itself: jobs stop once the plan is ready.
...
async function gather(edit, { signal, set }) {
  ...
  for (const [k, src] of edit.sources.entries()) {
    ...
    // Clip Studio projects are analyzed once and shared by every edit that uses them (moment ids carry the letter).
...
async function projectSource(projectId, letter) {
  ...
  if (!p?.source?.file) throw new Error(`Clip Studio project ${projectId} not found.`);
 ...
 * @param projectIds  Clip Studio projects to use as sources (source video + transcript reused, nothing copied)
...
/** Finished edits, newest first — for Clip Studio's scheduler library. */
...
/**
 * @param sounds  optional { addTrack } from lib/sounds.js. Pass it when mounting inside Clip Studio's process so
