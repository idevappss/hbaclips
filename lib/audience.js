...
export async function readSource(file) {
  ...
  if (ext === ".pdf") throw new Error("Read a PDF with readSourceParts, which hands it to Claude whole.");
  const raw = await fs.readFile(file, "utf8");
  return ext === ".vtt" || ext === ".srt" ? textFromSubtitles(raw) : clean(raw);
}

/**
 * What to send Claude for a source file: `{ text }` for anything readable here, or `{ document }` for a PDF, which
 * Claude reads itself — pages, slides, tables and all — so nothing is lost flattening it to text.
 */
export async function readSourceParts(file) {
  if (path.extname(file).toLowerCase() === ".pdf") {
    return { document: { media_type: "application/pdf", data: (await fs.readFile(file)).toString("base64") } };
  }
  return { text: await readSource(file) };
}
...
export async function ingestSource({ name, text, document = null, note = "" }) {
  await load();
  ...
  if (!document && body.length < 200) throw new Error("There's not enough in that file to learn from.");
    ...
    messages: [
      {
        ...
        content: document
          ? [
              { type: "document", source: { type: "base64", media_type: document.media_type, data: document.data }, title: name },
              { type: "text", text: `${previous}That document is the new source: ${name}${note ? ` (${note})` : ""}.\n\nWrite the brief.` },
            ]
          : `${previous}New source: ${name}${note ? ` (${note})` : ""}\n\n<source>\n${body}\n</source>\n\nWrite the brief.`,
      },
    ],
  });
  ...
  db.sources = [...(db.sources || []).filter((s) => s.name !== name), { name, note, chars: document ? null : body.length, at: new Date().toISOString() }].slice(-20);
  await persist();
  return db.brief;
}
